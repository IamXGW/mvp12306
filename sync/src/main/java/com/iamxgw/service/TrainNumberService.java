...
import com.iamxgw.common.TrainEsConstant;
...
import com.iamxgw.util.JsonMapper;
...
public class TrainNumberService {
    ...
    public void handle(List<CanalEntry.Column> columns, CanalEntry.EventType eventType) throws Exception {
        if (eventType != CanalEntry.EventType.UPDATE) {
            log.info("train number not update, not handle");
            return;
        }
        int trainNumberId = 0;
        ...
        log.info("trainNumber:{} detailList save es", trainNumber.getName());
    }
    ...
    private void saveES(List<TrainNumberDetail> detailList, TrainNumber trainNumber) throws Exception {
         ...
         */
        List<String> list = Lists.newArrayListWithCapacity(detailList.size() * detailList.size());
        if (list.size() == 1) {
        ...
        MultiGetResponse multiGetResponses = esClient.multiGet(multiGetRequest);
        ...
        for (MultiGetItemResponse itemResponse : multiGetResponses.getResponses()) {
            ...
            if (!getResponse.isExists() || map == null) {
                ...
                IndexRequest indexRequest = new IndexRequest(TrainEsConstant.INDEX, TrainEsConstant.TYPE, getResponse.getId())
                        .source(dataMap);
